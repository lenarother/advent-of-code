        ...
        if current == end:
            ...
            for n in neighbors(current):
                if grid[n] != '#' and in_grid(n, max_p) and n not in visited:
                    ...
                    visited.append(n)
                if collect_walls and grid[n] == '#' and in_grid(n, max_p) and not is_wall(n, max_p):
                    walls.append(n)
...
def get_cheating_path(grid, start, end, max_p, cheat):
    ...
    while paths:
        ...
        if current == end:
            ...
            for n in neighbors(current):
                if in_grid(n, max_p) and (n == cheat or grid[n] != '#') and n not in visited:
                    paths.append((n, length + 1))
                    visited.append(n)
    return min(ready)
