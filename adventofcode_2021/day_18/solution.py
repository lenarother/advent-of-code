...
def explode_left(left_part, lch):
    ...
    return rreplace(left_part, n, f'{new_n}', 1)


def explode_right(right_part, rch):
...
def explode_expr(expr):
    ...
    while expr:
        ...
        if open_count == 5:
            ...
            new_left = explode_left(left_part, lch)
            new_right = explode_right(right_part, rch)
...
def reduce_explode(expr):
    while 1:
        new_expr = explode_expr(expr)
        if new_expr == expr:
            return new_expr
        expr = new_expr
...
def reduce_expr(expr):
    while 1:
        expr = reduce_explode(expr)
        new_expr = split_expr(expr)
        if new_expr == expr:
            return new_expr
        expr = new_expr
